/**
 * Open-Source Voice Interface Component
 * 
 * A React component that provides voice interface capabilities for chatbots
 * using only open-source speech-to-text and text-to-speech engines.
 */

class OpenVoiceInterface extends React.Component {
  constructor(props) {
    super(props);
    ...
    this.loadEngineInfo = this.loadEngineInfo.bind(this);
  }
  ...
  /**
   * Load voice engine information
   */
  async loadEngineInfo() {
    try {
      const response = await fetch('/api/open-voice/models', {
        method: 'GET',
        headers: {
      ...
      if (!response.ok) {
        throw new Error('Failed to load engine information');
      }
      
      const data = await response.json();
      
      if (data.success) {
        this.setState({ engineInfo: data });
        
        if (this.props.onEngineInfoLoaded) {
          this.props.onEngineInfoLoaded(data);
        }
      }
    } catch (error) {
      console.error('Error loading engine information:', error);
      this.setState({ errorMessage: 'Failed to load engine information' });
    }
  }
  ...
  async loadVoiceSettings() {
    try {
      const response = await fetch(`/api/open-voice/chatbots/${this.props.chatbotId}/settings`, {
        method: 'GET',
        headers: {
      ...
      if (data.success && data.settings) {
        ...
        if (this.props.onSettingsLoaded) {
          this.props.onSettingsLoaded(data.settings);
        }
      }
    } catch (error) {
  ...
  async sendAudioToServer(audioBlob) {
    try {
      ...
      formData.append('sttEngine', settings.stt.engine);
      formData.append('ttsEngine', settings.tts.engine);
      ...
      // Send to server
      const response = await fetch(`/api/open-voice/chatbots/${this.props.chatbotId}/conversation`, {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('token')}`
        },
  ...
  render() {
    ...
    return (
      <div className="open-voice-interface">
        <button
        ...
        {this.props.showEngineInfo && engineInfo && (
          <div className="engine-info">
            <small>
              STT: {engineInfo.stt?.defaultEngine || 'Not loaded'} | 
              TTS: {engineInfo.tts?.defaultEngine || 'Not loaded'}
            </small>
          </div>
        )}
...
OpenVoiceInterface.propTypes = {
  ...
  onSettingsLoaded: PropTypes.func,
  onEngineInfoLoaded: PropTypes.func,
  onError: PropTypes.func
};

OpenVoiceInterface.defaultProps = {
  buttonSize: 'medium',
